...
*> \param[in] DY
*> \verbatim
*>          DY is COMPLEX*20 array, dimension (N)
...
*
            ...
            IF ( Y_PREC_STATE .EQ. BASE_RESIDUAL ) THEN
               CALL ZHEMV( UPLO, N, CMPLX(-1.0D+0), A, LDA, Y( 1, J ),
     $              1, CMPLX(1.0D+0), RES, 1 )
            ELSE IF ( Y_PREC_STATE .EQ. EXTRA_RESIDUAL ) THEN
               CALL BLAS_ZHEMV_X( UPLO2, N, CMPLX(-1.0D+0), A, LDA,
     $              Y( 1, J ), 1, CMPLX(1.0D+0), RES, 1, PREC_TYPE)
            ELSE
               CALL BLAS_ZHEMV2_X(UPLO2, N, CMPLX(-1.0D+0), A, LDA,
...
*
         CALL ZCOPY( N, B( 1, J ), 1, RES, 1 )
         CALL ZHEMV( UPLO, N, CMPLX(-1.0D+0), A, LDA, Y(1,J), 1,
     $        CMPLX(1.0D+0), RES, 1 )
